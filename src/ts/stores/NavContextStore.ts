...
export class NavContextStore extends StoreBase {
    ...
    constructor() {
        ...
        ResponsiveWidthStore.subscribe(() => {
            ...
            if (useStackNav !== this._isUsingStackNav) {
                ...
                this.navigateToTodoList('',undefined,true);
            }
        });

        if (this._isUsingStackNav) {
            const stackNavContext = new NavModels.StackRootNavContext();
            stackNavContext.stack.push(new NavModels.HomeViewNavContext());
            this._navContext = stackNavContext;
        } else {
            this._navContext = new NavModels.TodoRootNavContext(undefined,false,true);
        }
    }
    ...
    navigateToTodoList(selectedTodoId?: string, showNewTodoPanel = false,showHome= false,showTokenomics= false,showRoadMap= false,showWhitepaper= false) {
        this.setNavContext(NavActions.createTodoListContext(this._isUsingStackNav, selectedTodoId, showNewTodoPanel,showHome,showTokenomics,showRoadMap,showWhitepaper));
    }
